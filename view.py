...
end_time.set('The system will shut down in:\nThu, 01 Jan 1970, 00:00:00')  # Set the default value of the end time to a placeholder date and time 
count_time = StringVar()  # Variable to store the countdown time
count_time.set('00:00:00')  # Set the default value of the countdown time to a placeholder time
...
def add_components_to_main_window():
    ...
    title = Label(root, text='AutoShutdown', font=('Tahoma', 18, 'bold'))
    title.pack(pady=20)
    ...
    label = Label(root, text='Choose the time at which the system should be shut down.', font=('Tahoma', 8, 'normal'))
    label.pack()
    ...
    shutdown_after_label = Label(root, text='Shutdown after:', font=('Tahoma', 8, 'normal'))
    shutdown_after_label.pack()

    # Add example time
    model_time = ttk.Combobox(root, values=['5 min', '10 min', '20 min', '30 min', '1 hr', '2 hr', '3 hr'], font=('Tahoma', 8, 'normal'))
    model_time.pack(pady=10)

    # Add another button to set the time
    set_button = Button(root, text='Set', font=('Tahoma', 8, 'normal'), command=confirm_shutdown)
    set_button.pack()

    # Add another button to customize the time
    custom_button = Button(root, text='Custom time', font=('Tahoma', 8, 'normal'), command=add_components_to_customize_window)
...
def add_components_to_customize_window():
    ...
    prompt = Label(customize_window, text='Choose the time at which the system should be shut down:', font=('Tahoma', 8, 'normal'))
    prompt.pack()

    # Add the calendar to choose the date
    calendar = Calendar(customize_window, font=('Tahoma', 8, 'normal'))
    calendar.pack(pady=10)

    # Add the comboboxes to choose the time
    hours = ttk.Combobox(customize_window, values=[str(i).zfill(2)  for i in range(24)], font=('Tahoma', 8, 'normal'))
    hours.pack(pady=2)
    minutes = ttk.Combobox(customize_window, values=[str(i).zfill(2)  for i in range(60)], font=('Tahoma', 8, 'normal'))
    minutes.pack(pady=2)
    ...
    set_button = Button(customize_window, text='Set', font=('Tahoma', 8, 'normal'), command=confirm_shutdown_custom)
...
def add_components_to_countdown_window():
    ...
    countdown_label = Label(root, textvariable=end_time, font=('Tahoma', 8, 'normal'))
    countdown_label.pack(pady=10)

    # Add a label to display "Remaining time:"
    remaining_time_text = Label(root, text='Remaining time:', font=('Tahoma', 8, 'normal'))
    remaining_time_text.pack(pady=10)

    # Add a label to display the remaining time (this will be updated dynamically)
    remaining_time_label = Label(root, textvariable=count_time, font=('Tahoma', 40, 'bold'))
    remaining_time_label.pack(pady=20)

    # Add a button to cancel the shutdown
    cancel_button = Button(root, text='Cancel shutdown', font=('Tahoma', 8, 'normal'), command=cancel_shutdown)
...
def confirm_shutdown_custom():
    ...
    if not hour or not minute:
        messagebox.showwarning('Warning', 'WARNING 1: The hour or minute is empty')  # Show an error message if the hour or minute is empty
        return
    target_time = date.replace(year=int(year)+2000, month=int(month), day=int(day), hour=int(hour[0:2]), minute=int(minute[0:2]), second=0)
